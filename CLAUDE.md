...
- `/study/:unitCode` — 단원 챕터 목록(블로그 인덱스). 각 챕터 카드 클릭 시 챕터 본문으로 이동. GENERAL 단원의 빈칸 채우기는 이 페이지 하단에 접이식 섹션으로 함께 제공(단원 단위 유지). GENERAL 단원 카드/로드맵 노드(`/units/:code`)는 이 경로로 이동/리다이렉트한다.
- `/study/:unitCode/:sortOrder` — 챕터 본문 페이지. 마운트 시(로그인) 챕터 방문을 기록해 진도에 반영한다. 본문 h2/h3 를 스캔한 목차를 (1) 상단 토글형, (2) 우측 고정 패널 두 UI(`ChapterToc`)로 제공하고, 스크롤에 따라 현재 섹션을 강조(IntersectionObserver)하며 하단에 이전/다음 챕터 네비게이션을 둔다.
- 학습 콘텐츠는 단원당 `content` 1행 + `content_chapter` N개로 저장한다(통짜 body 아님). 진도는 "방문 챕터 / 전체 챕터"로 계산하며 저장된 퍼센트는 두지 않는다. 실습(PRACTICE) 단원은 `/units/:code`(미션 갤러리 + 실습 노트)를 유지하되, 노트는 챕터를 이어붙여 렌더링한다.
...
5. 실습 시뮬레이터(Python/DB/Docker/K8s) — 항목별로 순차 구현 — 진행 중. 인터랙티브 채점 실습은 리눅스 실습(SHELL 미션)만 완료. **8단계에서 7개 PRACTICE 단원 전체에 실습 노트(콘텐츠)를 먼저 채웠고**, 나머지 6종의 인터랙티브 실습 기능(Pyodide/sql.js/시뮬레이터 등)은 배포 후 진행 예정.
...
8. Docker Compose 배포 → Kubernetes 전환 — **진행 중(1차 HTTP 배포까지 완료)**:
   - **A. 실습 노트 추가 완료**: 7개 PRACTICE 단원(리눅스/쉘/서버구축/파이썬/데이터베이스/도커/K8s 실습) 전부 `content` 시드. 리눅스 실습은 미션 갤러리+풀이 화면 하단 노트, 나머지 6종은 "실습 기능 준비 중" 안내+노트 전체 렌더링. (K8s 실습은 전용 노트가 없어 GENERAL k8s 와 동일한 "쿠버네티스 실무" 노트를 사용 — 사용자 검토 대상)
   - **B. 이론/실습 이미지 반영 완료**: GENERAL 7개(cloud-intro·linux·shell·python·docker·security·k8s)와 PRACTICE 7개 노트의 이미지를 `frontend/public/assets/imgs/notes/<unit-code>/` 로 복사, `<img src>` 연결(총 318장). AWS·네트워크 GENERAL 은 노트에 이미지가 각각 214·75장으로 과다해 이번엔 미반영(사용자 검토 대상). `.learn-content` 이미지 스타일 보강.
   - **C. 배포**: `deploy/docker-compose.prod.yml`(nginx 80 단일 진입점, /api 프록시, mariadb 내부 전용) + `deploy/nginx/default.conf`(certbot 뼈대 포함). CORS·쿠키·API URL 을 도메인(`lib.solcho.com`)에 맞춰 환경변수화(`.env.production`). 런북: `docs/deploy.md`. HTTPS(443)는 다음 단계.
   - K8s 전환은 이후.
   - **학습 콘텐츠 챕터 구조 개선(배포 후 추가 작업)**: GENERAL/PRACTICE 학습 본문을 통짜 HTML(`content.body`)에서 **챕터(`content_chapter`)**로 분리했다. 파일(`resources/content/*.html`)은 그대로 두고 `ContentDataInitializer`가 기동 시 섹션 헤딩(h2 ≥ 2개면 h2, 아니면 h3) 기준으로 파싱해 챕터를 만든다(idempotent). 진도는 스크롤(`progress`)이 아니라 **챕터 방문(`chapter_progress`)** 기반("방문 챕터/전체 챕터")으로 바꿨다. 프론트에 챕터 인덱스/본문 페이지 + 목차(`ChapterToc`) 2종 UI를 추가했다. ⚠️ 스키마 변경(컬럼/테이블 드롭)은 `ddl-auto=update`가 처리하지 못하므로 DB 볼륨을 새로 만들어 반영한다(자세한 내용 `docs/db-schema.md`).
